...
const uint16_t  local_maxUrlLength        = 128;
const uint16_t  local_maxFieldNameLength  =  64;
const uint16_t  local_maxFieldValueLength = 256;
...
void HttpSvr::begin_noDHCP(uint16_t the_port)
{ 
  for (uint8_t sn = W5100::socket_begin; sn < W5100::socket_end; ++sn)
    prv_resetSocket(W5100::socket_cast(sn), the_port);
}

void HttpSvr::begin_noDHCP(int the_sdPinSS, int the_sdPinCS, uint16_t the_port)
{ my_sdSvr.begin(the_sdPinSS, the_sdPinCS); begin_noDHCP(the_port); }
                             ...
                             char *the_fieldValue, uint16_t the_fieldValueLen) const
{
  ...
  while (true)
  {
    ...
    if (ch == ':') break;
    if (the_fieldName && the_fieldNameLen && (u < the_fieldNameLen-1))
      the_fieldName[u++] = ch;
  }
  ...
  while (true)
  {
    if (!the_client.readByte(ch)) return false;
    if (ch == '\r') { the_client.unreadByte(ch); break; }
    if (the_fieldValue && the_fieldValueLen && (u < the_fieldValueLen-1))
      the_fieldValue[u++] = ch;
  }
...
bool HttpSvr::skipNextHeader(ClientProxy& the_client) const
{ return readNextHeader(the_client, 0,0,0,0); }
...
uint16_t HttpSvr::skipToBody(ClientProxy& the_client) const
{
  ...
  char sFieldName [strlen(HttpSvr_content_length) + 2];
  char sFieldValue[8];
...
bool HttpSvr::sendResFile(ClientProxy& the_client, const char * the_urlBuffer)
{
  ...
  if (my_sdSvr.resFileExists(the_urlBuffer))
  {
    ...
    const uint16_t uResBufferSize = 128;
...
{ 
  // 200 OK
  const char * msg = HttpSvr_HTTP_VERSION HttpSvr_SP HttpSvr_SC_200 HttpSvr_SP HttpSvr_RP_200 HttpSvr_CRLF
...
bool HttpSvr::sendResponseOkWithContent(ClientProxy& the_client, uint32_t the_size) const
{
  ...
  // 200 OK
  const char * msg01 = HttpSvr_HTTP_VERSION HttpSvr_SP HttpSvr_SC_200 HttpSvr_SP HttpSvr_RP_200 HttpSvr_CRLF;
  const char * msg02 = HttpSvr_header_server HttpSvr_CRLF;
  const char * msg03 = HttpSvr_header_content_type_html HttpSvr_CRLF;
  const char * msg04 = HttpSvr_header_content_length;
  const uint16_t msg04_len = strlen(msg04);
...
{
  // 400 Bad Request
  const char * msg = HttpSvr_HTTP_VERSION HttpSvr_SP HttpSvr_SC_400 HttpSvr_SP HttpSvr_RP_400 HttpSvr_CRLF
                     HttpSvr_header_server HttpSvr_CRLF
                     HttpSvr_CRLF;
  return prv_sendString(the_client, msg); 
}
...
{
  // 404 Not Found
  const char * msg = HttpSvr_HTTP_VERSION HttpSvr_SP HttpSvr_SC_404 HttpSvr_SP HttpSvr_RP_404 HttpSvr_CRLF
                     HttpSvr_header_server HttpSvr_CRLF
                     HttpSvr_CRLF;
  return prv_sendString(the_client, msg); 
}
...
bool HttpSvr::sendResponseMethodNotAllowed(ClientProxy& the_client) const
{
  ...
  const char * msg = HttpSvr_HTTP_VERSION HttpSvr_SP HttpSvr_SC_405 HttpSvr_SP HttpSvr_RP_405 HttpSvr_CRLF
                     HttpSvr_header_server HttpSvr_CRLF
                     HttpSvr_CRLF;
  return prv_sendString(the_client, msg); 
}
...
bool HttpSvr::sendResponseRequestUriTooLarge(ClientProxy& the_client) const
{
  ...
  const char * msg = HttpSvr_HTTP_VERSION HttpSvr_SP HttpSvr_SC_414 HttpSvr_SP HttpSvr_RP_414 HttpSvr_CRLF
                     HttpSvr_header_server HttpSvr_CRLF
                     HttpSvr_CRLF;
  return prv_sendString(the_client, msg); 
}
...
bool HttpSvr::sendResponseInternalServerError(ClientProxy& the_client) const
{
  ...
  const char * msg = HttpSvr_HTTP_VERSION HttpSvr_SP HttpSvr_SC_500 HttpSvr_SP HttpSvr_RP_500 HttpSvr_CRLF
                     HttpSvr_header_server HttpSvr_CRLF
                     HttpSvr_CRLF;
  return prv_sendString(the_client, msg); 
}
...
bool HttpSvr::prv_dispatchPOST(ClientProxy& the_client, const char * the_urlBuffer)
{
  ...
  if ((u >= smy_resMap_size) || (my_resMap[u].crc == 0))
  {
    ...
    const char * sMultipart = "multipart/form-data";
    ...
    const char * sBoundaryName= "boundary=";
    ...
    *sFilenameEnd = 0;
    ...
    const char * sLocalName = "upload.txt";
    if (SD.exists(const_cast<char *>(sLocalName))) SD.remove(const_cast<char *>(sLocalName));
...
bool HttpSvr::prv_sendString(ClientProxy& the_client, const char * the_str) const
{
  ...
  uint32_t u = 0;
  uint8_t *msg = (uint8_t *)the_str;
  while (*the_str)
  {
    u++; the_str++;
    if (u >= msg_size) { the_client.writeBuffer(msg, msg_size); u = 0; msg = (uint8_t *)the_str;}
  }
